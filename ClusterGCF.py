...
class ClusterGCF(object):
    def __init__(self, data_config, pretrain_data):
        # argument settings
        self.model_type = 'ClusterGCF'
        ...
        *********************************************************
        ...
        self.ua_embeddings, self.ia_embeddings, self.A_fold_hat_group_filter, self.user_group_embeddings = self._create_ca_gcf_embed()
        """
        *********************************************************
    ...
    def _create_ca_gcf_embed(self):
        ...
        user_group_embeddings_hidden_1 = tf.nn.leaky_relu(
            tf.matmul(user_group_embeddings_side, self.weights['W_gc_1']) + self.weights['b_gc_1'])
        user_group_embeddings_hidden_d1 = tf.nn.dropout(user_group_embeddings_hidden_1, 0.6)
        user_group_embeddings_sum = tf.matmul(user_group_embeddings_hidden_d1, self.weights['W_gc']) + self.weights[
            'b_gc']
        probability = self.gumbel_softmax(user_group_embeddings_sum,self.temperature, hard=False)
        # Matrix mask
        A_fold_hat_group, A_fold_hat_group_filter = self._split_A_hat_group(self.norm_adj, probability)
...
if __name__ == '__main__':
    ...
    *********************************************************
    ...
    model = ClusterGCF(data_config=config, pretrain_data=pretrain_data)
    ...
    *********************************************************
    ...
    for epoch in range(args.epoch):
        ...
        for idx in range(n_batch):
            ...
            _, batch_loss, batch_mf_loss, batch_emb_loss, batch_reg_loss, group_filter, user_group_embeddings = sess.run(
                [model.opt, model.loss, model.mf_loss, model.emb_loss, model.reg_loss, model.A_fold_hat_group_filter,
                 model.user_group_embeddings],
    ...
    f.write(
        'lr=%.4f, batch_size=%s groups=%s,regs=%s,temperature=%s,layer_size=%s\n\t%s\n'
        % (args.lr,args.batch_size, args.groups, args.regs, args.temperature,args.layer_size,
          final_perf))
    f.close()
